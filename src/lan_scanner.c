...
void lan_scanner_tick(LanScanner *scanner, uint64_t now_us) {
  ...
  uint32_t step_timeout_ms = scanner->config.connect_timeout_ms;
  if (step_timeout_ms == 0U) {
    step_timeout_ms = 1U;
  }
  if (step_timeout_ms > UI_PROBE_BUDGET_MS) {
    step_timeout_ms = UI_PROBE_BUDGET_MS;
  }
